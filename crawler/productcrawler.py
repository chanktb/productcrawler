...
def push_to_github(commit_message):
    """Tự động add, commit, và push tất cả thay đổi lên GitHub dùng GitPython."""
    pat = os.environ.get('CHANKTB_PAT')
    if not pat:
        print("\n[!] Cảnh báo: Biến môi trường CHANKTB_PAT chưa được thiết lập.")
        return

    print("\n--- Starting GitHub push process (using GitPython)... ---")
    try:
        repo = git.Repo(search_parent_directories=True)

        # --- SỬA ĐỔI: Sử dụng email NOREPLY và username GitHub của bạn ---
        # Thay thế bằng email NOREPLY và username GitHub của bạn
